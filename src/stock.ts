...
export class Stock {
    ...
    async addFurniture() {
        const furnitureData = await inquirer.prompt([
            ...
            { type: 'input', name: 'description', message: 'Enter furniture description:' },
            { type: 'input', name: 'material', message: 'Enter furniture material:' },
            { type: 'input', name: 'dimensions', message: 'Enter furniture dimensions:' },
            { type: 'number', name: 'price', message: 'Enter furniture price:' },
        ]);
        const furniture: Furniture = { id: Date.now().toString(), ...furnitureData };
    ...
    async deleteFurniture() {
        const furnitureId = await inquirer.prompt({ type: 'input', name: 'id', message: 'Enter furniture ID to delete:' });
    ...
    async updateFurniture() {
        const furnitureId = await inquirer.prompt({ type: 'input', name: 'id', message: 'Enter furniture ID to update:' });
        const newData = await inquirer.prompt([
            { type: 'input', name: 'name', message: 'Enter new furniture name:' },
            { type: 'input', name: 'description', message: 'Enter new furniture description:' },
            { type: 'input', name: 'material', message: 'Enter new furniture material:' },
            { type: 'input', name: 'dimensions', message: 'Enter new furniture dimensions:' },
            { type: 'number', name: 'price', message: 'Enter new furniture price:' },
        ]);
        this.db.get('furniture').value().find({ id: furnitureId.id }).assign(newData).write();
    }
    ...
    private searchFurnitureBy(filter: string, value: string) {
        return this.furniture.filter(f => f[filter].toLowerCase().includes(value.toLowerCase()));
    }
    ...
    async searchFurniture() {
        const searchCriteria = await inquirer.prompt([
            { type: 'list', name: 'filter', message: 'Choose search filter:', choices: ['name', 'description'] },
            { type: 'input', name: 'value', message: 'Enter search value:' },
        ]);
        const filteredFurniture = this.searchFurnitureBy(searchCriteria.filter, searchCriteria.value);
